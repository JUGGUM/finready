...
- ~~**Verifier 대상 범위 미정**~~ — **해소** (2026-08-18, 실측 뒷받침 2026-08-19).
  ...
  `docs/decisions/2026-08-18-explained-constraint-null-hole.md` "해소됨"
- **Coverage 레이턴시가 TRD §14 예산(12초)을 넘는다** — 예산은 **S1+S2 합계**이며,
  5개 시나리오 실측에서 **합계 15.5~32.9s로 한 번도 충족한 적이 없다**(classifier 10.8~23.3s).
  타임아웃을 60초로 올려 실패 모양만 바꿔뒀고 **근본 해결은 안 됐다.**
  TRD §14가 대응 수단으로 **부분 병렬화를 직접 명시**한다.
  `docs/decisions/2026-08-18-coverage-prompt-tuning.md` "레이턴시가 TRD §14 예산을 넘겼다"
- ⚠️ **30초 계약 한도 초과가 이미 발생했다 (미추적)** — `CONS_A_002` wall **33,244ms**
